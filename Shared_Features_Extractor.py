...
def gen_model(mode='adversarial'):
  if (mode!='adversarial'):
  	...
  	gen.add(keras.layers.Dense(800))
  	gen.add(keras.layers.LeakyReLU())
  	gen.add(keras.layers.Dense(640))
  	gen.add(keras.layers.LeakyReLU())
  	gen.add(keras.layers.Dense(1,activation='sigmoid'))
  	return gen
  else:
  	gen = keras.Sequential()
  	gen.add(keras.layers.InputLayer(input_shape=1280))
...
def fine_tune(mode='adversarial',Model_base_Directory=None,train_label=None,test_label=None,x_train_embed=None,x_test_embed=None,ct_train_embed=None,ct_test_embed=None):
	if (mode!='adversarial'):
		train_embed = np.concatenate((x_train_embed,ct_train_embed),axis==0)
  		...
  		mlp = gen_model('without adversarial')
  		...
  		hist = mlp.fit(x=train_embed,y=train_label,validation_data=(test_embed,test_label),verbose=1,epochs=100,batch_size=32,use_multiprocessing=True,callbacks=save)
	else:
